...
function listenToKeyboardEvents(ScrollableComponent: React$Component) {
  return class extends React.Component<KeyboardAwareHOCProps, KeyboardAwareHOCState>
    ...
    constructor(props: KeyboardAwareHOCProps) {
      ...
      const keyboardSpace: number = props.viewIsInsideTabBar ? _KAM_DEFAULT_TAB_BAR_HEIGHT : 0;
    ...
    getScrollResponder = () => this._rnkasv_keyboardView && this._rnkasv_keyboardView.getScrollResponder();
     ...
     */
    scrollToFocusedInput = (reactNode: any, extraHeight?: number, keyboardOpeningTime?: number) => {
    ...
    scrollIntoView = async (element: React.Element<*>, options: ScrollIntoViewOptions = {}) => {
                  ...
                  if (Platform.OS === 'ios') {
                    ...
                    if (textInputBottomPosition > keyboardPosition) {
                      ...
                      this.scrollForExtraHeightOnAndroid(
                        totalExtraHeight - (keyboardPosition - textInputBottomPosition)
                      );
                    }
                  }
                }
    ...
    _handleOnScroll = (e: SyntheticEvent<*> & { nativeEvent: { contentOffset: number } }) => {
      this.position = e.nativeEvent.contentOffset;
    };

    _handleRef = (ref: React.Component<*>) => {
      this._rnkasv_keyboardView = ref && ref.getNode ? ref.getNode() : ref;
    ...
    render() {
      ...
      if (Platform.OS === 'android' && enableOnAndroid) {
        newContentContainerStyle = [].concat(contentContainerStyle).concat({
          paddingBottom:
            ((contentContainerStyle || {}).paddingBottom || 0) + this.state.keyboardSpace
        });
      }
      return (
        <ScrollableComponent
